...
1. 搜索不要默认走 PowerShell 管道，优先 rg。
2. JSON 和日志不要硬解析，交给 node / python。
3. 命令一旦出现多层引号，就停下来写脚本。
4. 把这些规则写进 Codex 本地指令，别只靠临场提醒。
5. 验证时用简单稳定的命令，不要为了省一步写复杂一行流。
6. 全程使用中文回答。
